  ...
  useEffect(() => {
    const loadContent = async () => {
      try {
        ...
        if (cached) {
          const { paragraphs, timestamp } = JSON.parse(cached);
          if (Date.now() - timestamp < CACHE_DURATION) {
            setState({ paragraphs, isLoading: false });
            return;
          }
        }
        ...
        const paragraphs = [
          ...
          "I am currently developing an NQ swing bot and an SPX Put Spread bot, which I hope to test from Q3 2025....so stay tuned! And, yes, in case you are wondering, this dashboard updates live every 10 secs with new trades taken. As time goes on, I'll aim to improve the functionality and usefulness of this dashboard.",
          "Happy viewing/trading! And...if you have any questions or feedback, please reach out to @3_lines_smooth on twitter/x"
        ];
